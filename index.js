...
bot.start(async (ctx) => {
    ...
    const welcomeText = `<b>[ RUMAH NETFLIX AUTOMATION SYSTEM ]</b>\n\n` +
        `Selamat datang di sistem otomatisasi RUMAH NETFLIX. Bot ini berfungsi sebagai mesin extractor otomatis untuk mendapatkan cookies Netflix (Live) secara massal.\n\n` +
    ...
    try {
        ...
        if (allResults.length > 0) {
            ...
            const summaryLayout = 
                ...
                `</blockquote>\n\n` +
                `<b>English 🇬🇧 :</b>\n` +
                `<i>Successfully hit cookies, if you want to tidy up the cookie format and check live/dead cookies, please forward this file to the bot @ConvertCookiesRumahNetflix_bot so that it can be automatically sorted.</i>\n\n` +
                `<b>Indonesian 🇮🇩 :</b>\n` +
                `<i>Berhasil hit cookies, jika ingin merapihkan format cookies dan cek cookies live/dead silahkan forward file ini ke bot @ConvertCookiesRumahNetflix_bot agar otomatis di sortir</i>`;
